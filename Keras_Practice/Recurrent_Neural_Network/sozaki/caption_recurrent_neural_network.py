...
def create_model(num_unique_letters):
    model = Sequential()
    model.add(LSTM(128, return_sequences=True, input_shape=(max_len_of_sent, num_unique_letters)))
    model.add(LSTM(64))
...
def on_epoch_end(epoch, logs):
    if(epoch % epochs_until_test == 0):
        full_generated_text = ""
        print('Testing time!')
        test_text = "but anyway you guys are gonna see how th"
...
if __name__ == '__main__':
    ...
    # model.fit(x, y, batch_size=256, epochs=1000, verbose=1, callbacks=[LambdaCallback(on_epoch_end=on_epoch_end)])
